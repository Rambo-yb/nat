...
int ProtListPopHead(void* list_head, void* data, int data_size) {
	...
	((ListNode*) list_head)->next_node = cur_node->next_node;
    free(cur_node->data);
    free(cur_node);
...
void* ProtListGet(void* list_head, int index) {
    if (list_head == NULL || index > ProtListSize(list_head)) {
        return NULL;
    }
...
int ProtListRemove(void* list_head, int index) {
	...
	ListNode* pre_node = (ListNode*)list_head;
	ListNode* cur_node = pre_node->next_node;
	while (index--)
	{
		pre_node = cur_node;
		cur_node = cur_node->next_node;
